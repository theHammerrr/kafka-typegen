...
import type { PlatformaticConsumerLike, PlatformaticConsumerTransportOptions, PlatformaticMessage, PlatformaticMessagesStream } from './platformatic-types.js';
...
export function createPlatformaticConsumerTransport<TKey = unknown>(
  ...
  return {
    async onTopic(topicName, handler) {
      ...
      const stream = await consumer.consume({ ...(options.consumeOptions ?? {}), topics: [topicName] });
